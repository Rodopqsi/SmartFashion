...
export default function Profile(){
  ...
  // Normalize API base: ensure it includes trailing '/api'
  ...
  const [tab, setTab] = useState('datos') // 'datos' | 'correos' | 'password' | 'sesiones'
      ...
      {loading ? <div>Cargando...</div> : (
        <>
          ...
          <div style={{display:'flex', gap:8, flexWrap:'wrap', marginTop:12}}>
            <button onClick={()=>setTab('datos')} style={tabBtn(tab==='datos')}>Datos</button>
            <button onClick={()=>setTab('correos')} style={tabBtn(tab==='correos')}>Correos</button>
            <button onClick={()=>setTab('password')} style={tabBtn(tab==='password')}>Contraseña</button>
            <button onClick={()=>setTab('sesiones')} style={tabBtn(tab==='sesiones')}>Sesiones</button>
          </div>
          ...
          <section style={card}>
            ...
            <div style={{marginTop:12}}>
              <button onClick={saveProfile} style={btnPrimary}>Guardar cambios</button>
            </div>
          </section>
          )}
          ...
          {tab==='correos' && (
          <section style={card}>
            <h3 style={secTitle}>Correos vinculados</h3>
            <div style={{display:'flex', gap:8, flexWrap:'wrap'}}>
              {emails.map(e => (
                <div key={e.email} style={emailPill(e.is_primary)}>
                  <div>
                    <div style={{fontWeight:600}}>{e.email}</div>
                    <div style={{fontSize:12, color:'#666'}}>{e.is_primary ? 'Principal' : e.is_verified ? 'Verificado' : 'Sin verificar'}</div>
                  </div>
                  <div style={{display:'flex', gap:6}}>
                    {!e.is_primary && e.is_verified && <button onClick={()=>setPrimary(e.email)} style={btnSmall}>Hacer principal</button>}
                    {!e.is_primary && <button onClick={()=>deleteEmail(e.email)} style={btnSmallDanger}>Eliminar</button>}
                  </div>
                </div>
              ))}
            </div>
            <div style={{marginTop:12, display:'flex', gap:8, alignItems:'center'}}>
              ...
              <button onClick={addNewEmail} style={btnPrimary}>Agregar</button>
            </div>
            {verifyEmail.email && (
              <div style={{marginTop:8, display:'flex', gap:8, alignItems:'center'}}>
                ...
                <button onClick={verifyNewEmail} style={btnPrimary}>Verificar</button>
              </div>
            )}
          </section>
          )}
          ...
          <section style={card}>
            ...
            <div style={{marginTop:12, display:'flex', gap:10, alignItems:'center', flexWrap:'wrap'}}>
              <button onClick={startPasswordChange} style={btnPrimary}>Enviar código</button>
              ...
              <button onClick={confirmPasswordChange} style={btnPrimary}>Confirmar</button>
            </div>
          </section>
          )}
          ...
          <section style={card}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <h3 style={secTitle}>Sesiones</h3>
              <div style={{display:'flex', gap:8}}>
                <button onClick={loadSessions} style={btnSmall}>Refrescar</button>
                <button onClick={logoutAll} style={btnSmallDanger}>Cerrar todas</button>
              </div>
            </div>
            <div style={{display:'grid', gap:8}}>
              {sessions.map(s => (
                <div key={s.id} style={{border:'1px solid #eee', borderRadius:8, padding:'8px 10px'}}>
                  <div style={{fontSize:12, color:'#666'}}>{s.created_at} · {s.ip}</div>
                  <div style={{fontSize:12}}>{s.ua}</div>
                </div>
              ))}
              {!sessions.length && <div style={{color:'#666'}}>No hay sesiones listadas (usa Refrescar).</div>}
            </div>
          </section>
          )}
