...
with connection.cursor() as cursor:
    for stmt in create_statements:
        ...
        if not s:
            continue
        try:
            # If this statement is the ALTER that adds `status`, MySQL may not support
            # `ADD COLUMN IF NOT EXISTS` on all versions. Detect the intent and
            # perform a safe conditional alter using information_schema.
