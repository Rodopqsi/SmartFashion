...
export default function RatingStars({ value = 0, onChange, readOnly = false, name = 'rating', size = 'md' }){
    ...
    return (
      <span className="stars-readonly" aria-label={`Calificación ${value} de 5`}>
        {new Array(5).fill(0).map((_, i) => {
          const active = i < full
          return (
            <svg key={i} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style={{ fill: active ? 'var(--fill, #ffc73a)' : 'transparent', stroke: active ? 'var(--fill, #ffc73a)' : 'var(--stroke, #666)' }}>
  ...
  // interactive radios in reverse order
  return (
