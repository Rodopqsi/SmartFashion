...
# file extensions -> language category
EXT_LANG = {
    '.py': 'python',
...
# keywords to preserve in comments
...
def process_file(path: Path):
    ...
    # handle per language
    if lang == 'python':
        # remove # comments unless preserve
        ...
        for ln in lines:
            if ln.lstrip().startswith('#'):
                if contains_preserve(ln):
                    ...
                    changed = True
                continue
            # inline comment: keep code and strip trailing #comment if not preserve
            if '#' in ln:
                parts = ln.split('#',1)
        ...
        # remove -- comments and /* */ unless preserve
        def repl_sql_line(m):
            ln = m.group(0)
            if contains_preserve(ln):
                return ln
...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(ROOT):
        ...
        for f in files:
            p = Path(root) / f
            # skip backups and script itself
